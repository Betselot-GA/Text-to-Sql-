...
- **Database**: Uses the **bike store** DuckDB database (brands, categories, customers, order_items, orders, products, staffs, stocks, stores). The database is loaded from Kaggle via `db/bike_store.py`.

### Multi-Agent Pipeline

The agent uses a **Selector → Decomposer → Refiner** pipeline:

1. **Selector**: Chooses which database tables are relevant to the question (e.g. for “most expensive product” it selects the `products` table).
2. **Decomposer**: Generates an initial SQL query (including subqueries when needed, e.g. “above average”).
3. **Refiner**: Executes the SQL on the database. If execution fails, it uses the error message to ask the LLM for a corrected query and retries (up to 3 attempts). The fallback query `SELECT NULL WHERE FALSE` is **not** treated as success, so the pipeline does not “succeed” when it could not produce a real answer.
...
| **Streamlit** (optional) | Alternative web UI via `streamlit run app.py` if `app.py` is present. |

### Optional: Microsoft SQL Server

- If `SQL_SERVER_CONNECTION_STRING` is set, the agent can use **Microsoft SQL Server** for schema and execution (T-SQL, e.g. `TOP N` instead of `LIMIT N`). Requires `pyodbc`.
...
- **Single execution path**: All execution goes through a central helper (DuckDB or SQL Server), so errors and fixes are consistent.
...
├── agent.py              # QueryWriter + Selector/Decomposer/Refiner pipeline
├── main.py               # CLI entry point
├── runtime.txt           # Python version (e.g. python-3.11.9)
├── requirements.txt     # Python dependencies (install in venv)
├── Report.md            # This file
├── README.md            # Competition and general documentation
├── db/
│   ├── __init__.py
│   └── bike_store.py    # Load bike store dataset into DuckDB
├── backend/
│   ├── __init__.py
│   ├── server.py        # FastAPI: chats, ask, ask-start, ask-status (polling), ask-stream
...
The first run of `main.py` will download the bike store dataset using this file.
...
- The first time, this downloads the dataset (Kaggle) and creates `bike_store.db` in the project folder.
- When you see the “Enter your question” prompt, you can type a test question or type `quit` to exit.

After `bike_store.db` exists, you do not need to run `main.py` again just to create the DB; you can start the API or React flow directly.
...
| **Generated SQL is `SELECT NULL WHERE FALSE`** | The agent could not produce a valid query. Check: (1) Ollama is running and a model is pulled (e.g. `ollama pull llama3.2`). (2) `bike_store.db` exists (run `python main.py` once). (3) In the UI, open the turn, click **“Show pipeline steps”**, and read the red warning for more context. |
| **`streamlit` / `python` not recognized** | Use the venv’s executable: `.\venv\Scripts\python.exe -m streamlit run app.py` or `.\venv\Scripts\python.exe main.py` (Windows). |
